...
def spacy_tokenizer(
	...
	if method == "unigram":
		...
		for doc in docs:
			doc = nlp(doc)
			tokens = [token.text.lower() for token in doc] if lowercase else [token.text for token in doc]  # Changed line
					...
					if clause_remove_conj:
						...
						for i, word in enumerate(words):
							if not (word.tag_ == "CC" and i == len(words) - 1):
								chunk.append(word.text)  # Changed line
						chunk = " ".join(chunk)  # Changed line
					else:
