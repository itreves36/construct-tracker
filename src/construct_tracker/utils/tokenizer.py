...
def spacy_tokenizer(
	...
	if method == "word":
		tokens_for_all_docs = [
			[token.text.lower() if lowercase else token.text for token in nlp(doc)]
			for doc in docs
		]
		...
		for doc in tqdm.tqdm(nlp.pipe(docs, batch_size=2048), position=0):
			...
			for sent in doc.sents:
				...
				if clause_remove_conj:
					unseen = [
						word for i, word in enumerate(unseen)
		...
		docs_clauses_clean = [
			[clause.replace(' ,', ',').replace(' .', '.').replace(' !', '!').replace(' ?', '?').replace(" '", "'").replace('  ', ' ').strip(', ') for clause in doc]
